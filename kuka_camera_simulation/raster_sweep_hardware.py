...
class RasterSweepNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('require_face', False)
        # C2: movement pause/resume
        self.declare_parameter('movement_threshold', 999.0)  # effectively disabled
        self.declare_parameter('settle_tolerance', 0.02)    # m, 2cm band for real hardware
        self.declare_parameter('settle_frames', 5)          # ticks of stillness
        ...
        self.face_x = FACE_X  # updated live from depth camera
        self.have_face = False
        self.ref_face_y = REF_FACE_Y  # overwritten on first face reading
        self.ref_face_z = REF_FACE_Z
        # C2 state
        self.paused = False
    ...
    def face_cb(self, msg):
        # Only update when face is actually detected
        if msg.z > -0.01 and msg.z < 0.01 and msg.y > -0.01 and msg.y < 0.01:
            return  # (0,0,0) = no face, ignore
        # Store live face X distance from depth camera
        self.face_x = msg.x
        # Auto-initialize reference on first reading
        if not self.have_face:
            self.ref_face_y = msg.y
            self.ref_face_z = msg.z
            self.get_logger().info(
                f'Face reference set: y={msg.y:.3f}, z={msg.z:.3f}, x={msg.x:.3f}')
        self.face_offset_y = msg.y - self.ref_face_y
        self.face_offset_z = msg.z - self.ref_face_z
        self.have_face = True
    ...
    def project_target(self, ty, tz):
        ...
        wx = self.face_x - t.x  # live depth from RealSense
        wy = ty - t.y
        wz = tz - t.z
        ...
        if cx <= 0.05:
            self.get_logger().warn(f'cx={cx:.3f} too small - face behind camera?', throttle_duration_sec=2.0)
            return None
        px = FOCAL_PX * (-cy) / cx
        py = FOCAL_PX * (-cz) / cx
        return (px, py)
